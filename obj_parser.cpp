...
namespace jep
{
	...
	std::vector<float> mesh_data::getInterleaveData() const
	{
	...
	std::vector<float> mesh_data::getIndexedVertexData() const {
	...
	std::vector<float> mesh_data::getIndexedVertexData(std::vector<unsigned short> &indices) const {
		...
		map<unsigned short, vertex_data > vertexMap;
		...
		for (const auto& vertex : all_vertices) {
			...
			if (!match_found) {
				unsigned short new_index = vertexMap.size();
				...
				vertexMap.insert(std::pair<unsigned short, vertex_data>(new_index, vertex));
			}
		}
...
//		std::vector<int> shared_edges;
		...
		for (auto i : edges) {
			...
			for (auto j : edges) {
                ...
                if (sharedEdges.find(j.first) != sharedEdges.end()) {
                    continue;
                }

                if (uniqueEdges.find(j.first) != uniqueEdges.end()) {
                    continue;
                }
				...
				if ((i_edge.second == j_edge.first && i_edge.second == j_edge.second) ||
                    ...
                    sharedEdges.emplace(i.first);
                    sharedEdges.emplace(j.first);
					unique = false;
					break;
				}
			}
	...
	std::vector<vertex_data> createFaceFromVertices(const std::vector<int> &indices, const std::vector<vertex_data> &vertices) {
		...
		for (int index : indices) {
			try {
				returned_vertices.push_back(vertices.at(index));
	...
	void addDataToMesh(mesh_data &mesh, const std::vector<vertex_data> &vertices) {
		for (const auto & vertex : vertices) {
			mesh.addVData(vertex.getVData());
			mesh.addVTData(vertex.getVTData());
			mesh.addVNData(vertex.getVNData());
		}
	}
	...
	obj_contents::obj_contents(const char* obj_file) {
		...
		while (!file.eof()) {
            ...
            if (type == UNDEFINED_DATA_TYPE) {
                continue;
            } else if (type == OBJ_G) { //"g" prefix indicates the previous geometry data has ended
			...
			if (type == OBJ_V || type == OBJ_VT || type == OBJ_VN || type == OBJ_VP) {
				...
				for (auto & i : extracted_face_data) {
					int v_index = 0;
					int vt_index = 0;
					int vn_index = 0;
					int vp_index = 0;
	...
	{
		const vertex_data& v_data_0 = face_data[0];
		const vertex_data& v_data_1 = face_data[1];
		const vertex_data& v_data_2 = face_data[2];
	...
	std::vector<float> extractFloats(const string &s)
	{
		...
		for (int i = 0; i < s.size(); i++) {
            ...
            if (i == s.size() - 1 && !current_string.empty()) {
                add_string = true;
            }
	...
	map<string, std::shared_ptr<material_data> > generateMaterials(
	...
	std::vector<float> material_data::getData(DATA_TYPE dt) const {
	...
	mtl_contents::mtl_contents(const char* mtl_file, std::shared_ptr<texture_handler> &textures, const std::shared_ptr<ogl_context> &context) {
		...
		map<string, std::shared_ptr<material_data> >::iterator current_material;
