...
namespace jep
{
	...
	public:
        ...
        void setUniform3fv(const char* name, int count, const vec3& value);

        void setUniform4fv(const char* name, int count, const vec4& value);

        void setUniformMatrix4fv(const char* name, int count, bool transpose, const mat4& matrix);
	...
	private:
		...
		std::map<string, std::shared_ptr<GLint> > glint_map;
	...
	*/
	...
	public:
		...
		static_text(string s, text_justification tj, const std::shared_ptr<text_handler> &text,
	...
	private:
		...
		map<string, std::shared_ptr<GLuint> > font_map;
	};
	...
	public:
		...
		std::shared_ptr<GLuint> addTextureByFilename(const string &texture_handle, const string &file_name);
		std::shared_ptr<GLuint> addTextureByPath(const string &texture_handle, const string &file_path);
		std::shared_ptr<GLuint> getTexture(const string &texture_handle);
	...
	private:
		// map handle, gluint
		std::map<string, std::shared_ptr<GLuint> > map_gluints;
	...
	public:
		...
		const map<int, std::vector<float> > getAllRawVData() const { return raw_v_data; }
		const map<int, std::vector<float> > getAllRawVTData() const { return raw_vt_data; }
		const map<int, std::vector<float> > getAllRawVNData() const { return raw_vn_data; }
		const map<int, std::vector<float> > getAllRawVPData() const { return raw_vp_data; }
		...
		std::vector<string> getErrors() const { return error_log; }
	...
	private:
		...
		//uses std::vector<float> because # of floats per vertex varies
		//data direct from obj file, unformatted
		map<int, std::vector<float> > raw_v_data;
		map<int, std::vector<float> > raw_vt_data;
		map<int, std::vector<float> > raw_vn_data;
		map<int, std::vector<float> > raw_vp_data;
		string mtl_filename;
	...
	public:
		...
		std::shared_ptr<GLuint> getGluint(const string &map_handle) const;
		...
		bool overrideMap(const string &map_handle, const std::shared_ptr<GLuint> &new_gluint);
	...
	public:
		...
		const map<string, std::shared_ptr<material_data> >  getMaterials() const { return materials; }

	private:
		std::vector<string> error_log;
		map<string, std::shared_ptr<material_data> > materials;
	};
